"""
Loop-defined Template System for Dynamic Path Generation
...
def scan_and_match_files(base_dir: str,
                         ...
                         variable_combinations: Dict[str, List[Any]],
                         log_callback=None,
                         path_template: str = "") -> Tuple[List[Tuple[str, Dict[str, Any], str]], Dict[str, Any]]:
    """
    Scan directory recursively and match files against filename template and path template
    ...
    # Extract path variables if path_template is provided
    path_variables = []
    if path_template:
        path_var_pattern = r'\{([^}]+)\}'
        path_variables = re.findall(path_var_pattern, path_template)
        log(f"Path template: {path_template}")
        log(f"Path variables: {path_variables}")
    ...
    for file_path in all_files:
        ...
        # Extract variables from path if path_template is provided
        path_parsed_vars = {}
        if path_template:
            ...
            try:
                ...
                # Build path template pattern by replacing variables with regex groups
                path_pattern = path_template
                path_var_to_group = {}
                for var in path_variables:
                    if var not in path_var_to_group:
                        # Replace {var} with a named regex group that matches non-slash characters
                        path_pattern = path_pattern.replace(f"{{{var}}}", f"(?P<path_{var}>[^/]+)")
                        path_var_to_group[var] = f"path_{var}"
                
                # Escape literal parts (slashes) but keep regex groups
                # First, temporarily mark regex groups
                temp_markers = {}
                for var in path_variables:
                    group_name = path_var_to_group[var]
                    marker = f"__TEMP_MARKER_{var}__"
                    temp_markers[marker] = f"(?P<{group_name}>[^/]+)"
                    path_pattern = path_pattern.replace(f"(?P<path_{var}>[^/]+)", marker)
                
                # Escape slashes
                path_pattern = re.escape(path_pattern)
                
                # Restore regex groups
                for marker, group_pattern in temp_markers.items():
                    path_pattern = path_pattern.replace(re.escape(marker), group_pattern)
                
                # The pattern should match anywhere in the path (not just start)
                # Use search instead of match to allow extra path components before/after
                path_match = re.search(path_pattern, rel_dir)
                if path_match:
                    for var in path_variables:
                        group_name = path_var_to_group[var]
                        if group_name in path_match.groupdict():
                            path_parsed_vars[var] = path_match.group(group_name)
            except Exception as e:
                # If path parsing fails, continue without path variables
                if log_callback:
                    log_callback(f"Warning: Could not parse path for {file_path}: {e}")
        ...
        # Merge path variables with filename variables (path variables take precedence if conflict)
        for var, value in path_parsed_vars.items():
            parsed_vars[var] = value
