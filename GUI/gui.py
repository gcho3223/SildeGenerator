...
from GUI.gui_usrDefined import (
    create_usrDefined_options_ui, create_usrDefined_objects_ui, 
    create_usrDefined_variables_ui, create_usrDefined_preview_ui,
    update_usrDefined_preview
)
...
class GeneratorThread(QThread):
    ...
    def __init__(self, mode, systematic, input_dir, output_file, selected_objects, selected_steps, 
                 ...
                 user_settings=None, usrDefined_path_template="", usrDefined_filename_template="", usrDefined_variables=None,
                 usrDefined_row_values=None, usrDefined_column_values=None, usrDefined_scan_mode=False):
        super().__init__()
        self.mode = mode
        ...
        self.usrDefined_path_template = usrDefined_path_template
        self.usrDefined_filename_template = usrDefined_filename_template
        self.usrDefined_variables = usrDefined_variables if usrDefined_variables is not None else {}
        self.usrDefined_row_values = usrDefined_row_values if usrDefined_row_values is not None else []
        self.usrDefined_column_values = usrDefined_column_values if usrDefined_column_values is not None else []
        self.usrDefined_scan_mode = usrDefined_scan_mode
        ...
        try:
            ...
            if self.mode == "cpv":
                ...
                if not self.isInterruptionRequested():
                    ...
                    self.log_signal.emit("Generation cancelled by user")
                    self.finished_signal.emit(False, "Generation cancelled")
            elif self.mode == "usrDefined":
                from GUI.gui_usrDefined import run_usrDefined_generation
                success, message = run_usrDefined_generation(self)
                if not self.isInterruptionRequested():
                    self.finished_signal.emit(success, message)
                else:
                    self.log_signal.emit("Generation cancelled by user")
...
class KeynoteSlideGeneratorGUI(QMainWindow):
    def __init__(self):
        super().__init__()
        # Window settings
        self.setWindowTitle("Silde Maker for Keynote v2.0")
    ...
    def init_ui(self):
        ...
        # Initially hidden (will be shown in User-defined mode)
        ...
        mode_layout.addWidget(self.usrDefined_radio)
        mode_layout.addStretch()
        # Preset UI (for User-defined mode)
        ...
        # ============ Directory Settings ============
        (dir_group, self.input_dir_edit, self.output_file_edit) = create_directory_ui(self)
        ...
        (self.usrDefined_options_group, self.usrDefined_file_format_combo, 
         self.usrDefined_path_template_input, self.usrDefined_filename_template_input,
         self.usrDefined_scan_mode_check) = create_usrDefined_options_ui(self)
        # Connect template input changes to update variable inputs
        self.usrDefined_path_template_input.textChanged.connect(self.update_usrDefined_template_info)
        self.usrDefined_filename_template_input.textChanged.connect(self.update_usrDefined_template_info)
        # Connect scan mode checkbox to update SCAN button visibility and path template input state
        self.usrDefined_scan_mode_check.stateChanged.connect(self._update_scan_button_visibility)
        self.usrDefined_scan_mode_check.stateChanged.connect(self._update_path_template_input_state)
        # Initialize path template input state
        self._update_path_template_input_state()
        main_layout.addWidget(self.usrDefined_options_group)
        self.usrDefined_options_group.setVisible(False)
        ...
        (self.usrDefined_objects_widget, usrDefined_object_inputs) = create_usrDefined_objects_ui(self)
        # Store separately for User-defined mode (text input instead of checkboxes)
        self.object_inputs = usrDefined_object_inputs
        ...
        object_layout.addWidget(self.usrDefined_objects_widget)
        self.usrDefined_objects_widget.setVisible(False)
        
        (self.usrDefined_variables_widget, self.usrDefined_variable_inputs) = create_usrDefined_variables_ui(self)
        object_layout.addWidget(self.usrDefined_variables_widget)
        self.usrDefined_variables_widget.setVisible(False)
        ...
        # User-defined Steps (no longer used - steps are handled in Variables section)
        ...
        # Preview Section (DRC and User-defined)
        ...
        # User-defined Preview Section
        (self.usrDefined_preview_group, self.usrDefined_preview_cells, 
         self.usrDefined_arrangement_combo, self.usrDefined_row_inputs_container,
         self.usrDefined_row_inputs_layout, self.usrDefined_column_input,
         self.usrDefined_preview_grid_layout, self.usrDefined_drc_plot_check,
         self.usrDefined_drc_energy_container, self.usrDefined_energy_checkboxes_layout,
         self.usrDefined_drc_controls_container, self.usrDefined_slide_axis_combo,
         self.usrDefined_grid_axis_combo, self.usrDefined_page_tabs) = create_usrDefined_preview_ui(self)
        left_side_layout.addWidget(self.usrDefined_preview_group)
        self.usrDefined_preview_group.setVisible(False)
        
        # Store row inputs dictionary
        self.usrDefined_row_inputs = {}
        
        # Store energy checks dictionary for DRC-style preview
        self.usrDefined_energy_checks = {}
        
        # Connect arrangement change to update row inputs
        self.usrDefined_arrangement_combo.currentTextChanged.connect(self.on_usrDefined_arrangement_changed)
        # Connect column input changes to update preview
        self.usrDefined_column_input.textChanged.connect(self.update_usrDefined_preview)
        # Connect DRC plot checkbox to toggle preview style
        self.usrDefined_drc_plot_check.stateChanged.connect(self.on_usrDefined_drc_plot_changed)
        # Connect axis dropdowns to update preview
        self.usrDefined_slide_axis_combo.currentTextChanged.connect(self.on_usrDefined_axis_changed)
        self.usrDefined_grid_axis_combo.currentTextChanged.connect(self.on_usrDefined_axis_changed)
        self.usrDefined_page_tabs.currentChanged.connect(self.on_usrDefined_page_changed)
        
        # Store scan results for DRC plot mode
        self.usrDefined_scan_results = None
        self.usrDefined_confirmed_scan_results = None
        
        # Initialize row inputs based on default arrangement
        self.on_usrDefined_arrangement_changed("2*3")
        ...
        # Connect User-defined mode select all checkboxes
        if hasattr(self, 'select_all_usrDefined_check'):
            self.select_all_usrDefined_check.toggled.connect(
                lambda checked: self._toggle_all_usrDefined_objects(checked)
            )
        # User-defined steps select all checkbox removed (steps are now in Variables section)
    ...
    def on_mode_change(self):
        """Handle mode change between CPV, DRC, and User-defined"""
        if self.cpv_radio.isChecked():
            ...
            self.preview_group.setVisible(False)
            self.usrDefined_preview_group.setVisible(False)  # Hide User-defined preview in CPV mode
            ...
            self.usrDefined_preview_group.setVisible(False)  # Hide User-defined preview in DRC mode
            ...
            self.mode = "usrDefined"
            self.systematic_check.setChecked(False)
            self.systematic_check.setEnabled(False)
            self.systematic_check.setVisible(False)  # Hide systematic checkbox in User-defined mode
            # Show preset UI in User-defined mode
            ...
            self.year_channel_group.setVisible(False)
            self.drc_options_group.setVisible(False)
            self.usrDefined_options_group.setVisible(True)
            ...
            self.cpv_objects_widget.setVisible(False)
            self.drc_channels_widget.setVisible(False)
            self.usrDefined_objects_widget.setVisible(False)  # Hide Objects section
            self.usrDefined_variables_widget.setVisible(True)
            ...
            self.update_usrDefined_template_info()
            # LeftLayout: Preview (User-defined mode)
            self.left_layout.setTitle("")
            self.left_layout.setVisible(False)  # Hide empty left_layout in User-defined mode
            self.cpv_steps_widget.setVisible(False)
            self.preview_group.setVisible(False)
            self.usrDefined_preview_group.setVisible(True)
            # Hide header label and select all checkbox for User-defined mode
            self.header_label.setVisible(False)
            self.select_all_steps_check.setVisible(False)
            # Update preview when switching to User-defined mode
            update_usrDefined_preview(self)
    ...
    def update_usrDefined_template_info(self):
        """Update user-defined variable input fields based on templates"""
        # Get templates from input fields
        path_template = self.usrDefined_path_template_input.text().strip()
        filename_template = self.usrDefined_filename_template_input.text().strip()
        ...
        if not path_template:
            ...
            self.usrDefined_path_template_input.setText(path_template)
        if not filename_template:
            filename_template = default_filename_template
            self.usrDefined_filename_template_input.setText(filename_template)
        
        # Extract variables from templates and update variable inputs dynamically
        self._update_usrDefined_variable_inputs(path_template, filename_template)
    
    def _update_usrDefined_variable_inputs(self, path_template, filename_template):
        ...
        try:
            ...
            self.usrDefined_path_variables = path_variables
            ...
            if hasattr(self, 'usrDefined_variables_widget'):
                layout = self.usrDefined_variables_widget.layout()
                if layout:
                    # Clear all items
                    while layout.count():
                ...
                if path_variables:
                    ...
                    self.usrDefined_variable_inputs = {}
                    ...
                    for var_name in path_variables:
                        ...
                        self.usrDefined_variable_inputs[var_name] = var_input
                    
                    layout.addLayout(path_form)
                    
                    # Force layout update
                    self.usrDefined_variables_widget.update()
                    self.usrDefined_variables_widget.repaint()
                ...
                if filename_variables:
                    ...
                    for var_name in filename_variables:
                        ...
                        if var_name in self.usrDefined_variable_inputs:
                            # Get existing input from path template
                            var_input = self.usrDefined_variable_inputs[var_name]
                ...
                self.usrDefined_variables_widget.repaint()
                if hasattr(self, 'usrDefined_variables_widget'):
                    self.usrDefined_variables_widget.show()
    ...
    def on_usrDefined_arrangement_changed(self, arrangement):
        """Handle arrangement change in User-defined mode"""
        from GUI.gui_usrDefined import update_usrDefined_row_inputs
        update_usrDefined_row_inputs(self, arrangement)
        self.update_usrDefined_preview()
    
    def update_usrDefined_preview(self):
        """Update User-defined preview - delegates to gui_usrDefined module"""
        from GUI.gui_usrDefined import update_usrDefined_preview
        update_usrDefined_preview(self)
    
    def on_usrDefined_drc_plot_changed(self, state):
        """Handle DRC plot checkbox change in User-defined mode"""
        drc_plot_enabled = (state == 2)  # Qt.Checked = 2
        
        if drc_plot_enabled:
            # Show DRC controls (axis dropdowns and tabs)
            if hasattr(self, 'usrDefined_drc_controls_container'):
                self.usrDefined_drc_controls_container.setVisible(True)
            
            # Hide row/column inputs container
            if hasattr(self, 'usrDefined_row_inputs_container'):
                self.usrDefined_row_inputs_container.setVisible(False)
            
            # Show info label
            if not hasattr(self, 'usrDefined_drc_info_label'):
                ...
                if hasattr(self, 'usrDefined_preview_group'):
                    preview_layout = self.usrDefined_preview_group.layout()
                    if preview_layout:
                        ...
                        for i in range(preview_layout.count()):
                            ...
                            if item and item.widget():
                                widget = item.widget()
                                if hasattr(widget, 'layout') and widget.layout() == self.usrDefined_preview_grid_layout:
                                    grid_widget = widget
                                    break
                        if grid_widget:
                ...
                self.usrDefined_drc_info_label = info_label
            else:
                self.usrDefined_drc_info_label.setVisible(True)
            
            # Update axis dropdowns with available variables
            self._update_usrDefined_axis_dropdowns()
            
            # Update preview (DRC mode)
            if self.usrDefined_confirmed_scan_results:
                self._update_usrDefined_drc_preview()
        else:
            # DRC plot disabled - hide DRC controls and show row/column inputs
            if hasattr(self, 'usrDefined_drc_controls_container'):
                self.usrDefined_drc_controls_container.setVisible(False)
            
            # Show row/column inputs container
            if hasattr(self, 'usrDefined_row_inputs_container'):
                self.usrDefined_row_inputs_container.setVisible(True)
            
            # Hide info label
            if hasattr(self, 'usrDefined_drc_info_label'):
                self.usrDefined_drc_info_label.setVisible(False)
            
            # Clear and hide tabs
            if hasattr(self, 'usrDefined_page_tabs'):
                # Block signals to prevent recursion
                self.usrDefined_page_tabs.blockSignals(True)
                # Clear all tabs
                while self.usrDefined_page_tabs.count():
                    self.usrDefined_page_tabs.removeTab(0)
                # Hide tabs
                self.usrDefined_page_tabs.setVisible(False)
                self.usrDefined_page_tabs.blockSignals(False)
            
            # Clear tab data
            if hasattr(self, 'usrDefined_tab_grids'):
                for tab_cells in self.usrDefined_tab_cells:
                    for cell in tab_cells:
                        if cell:
                            cell.deleteLater()
                self.usrDefined_tab_grids.clear()
                self.usrDefined_tab_cells.clear()
            
            # Use normal preview (row/col based)
            self.update_usrDefined_preview()
    
    def on_usrDefined_axis_changed(self):
        """Handle axis dropdown change"""
        # Only handle axis changes when DRC plot is enabled
        if hasattr(self, 'usrDefined_drc_plot_check') and self.usrDefined_drc_plot_check.isChecked():
            if self.usrDefined_confirmed_scan_results:
                self._update_usrDefined_drc_preview()
        # If DRC plot is not enabled, axis changes are ignored (use normal preview)
    
    def on_usrDefined_page_changed(self, index):
    ...
    def _update_usrDefined_drc_preview(self):
        """Update preview for DRC plot mode based on slide axis and grid axis"""
        if not hasattr(self, 'usrDefined_confirmed_scan_results') or not self.usrDefined_confirmed_scan_results:
            return
        
        if not hasattr(self, 'usrDefined_slide_axis_combo') or not hasattr(self, 'usrDefined_grid_axis_combo'):
            return
        
        slide_axis = self.usrDefined_slide_axis_combo.currentText()
        grid_axis = self.usrDefined_grid_axis_combo.currentText()
        
        if not slide_axis or not grid_axis or slide_axis == grid_axis:
            # Invalid selection - clear preview and hide main grid
            if hasattr(self, 'usrDefined_preview_cells'):
                for cell in self.usrDefined_preview_cells:
                    if cell:
                        cell.deleteLater()
                self.usrDefined_preview_cells.clear()
            if hasattr(self, 'usrDefined_preview_grid_layout'):
                while self.usrDefined_preview_grid_layout.count():
                    item = self.usrDefined_preview_grid_layout.takeAt(0)
                    if item.widget():
                        item.widget().deleteLater()
        ...
        # Get arrangement
        if hasattr(self, 'usrDefined_arrangement_combo'):
            arrangement = self.usrDefined_arrangement_combo.currentText()
            try:
        ...
        # Get scan results
        scan_results = self.usrDefined_confirmed_scan_results
        ...
        if hasattr(self, 'usrDefined_page_tabs'):
            ...
            if not hasattr(self, 'usrDefined_tab_grids'):
                self.usrDefined_tab_grids = []
            if not hasattr(self, 'usrDefined_tab_cells'):
                self.usrDefined_tab_cells = []
            ...
            for slide_idx, slide_value in enumerate(slide_axis_values):
                ...
                self.usrDefined_page_tabs.addTab(tab_widget, f"{slide_axis}={slide_value}")
                
                # Store grid layout
                self.usrDefined_tab_grids.append(tab_grid_layout)
                self.usrDefined_tab_cells.append([])
                ...
                for grid_value in grid_axis_values:
                    ...
                    # Create cell
                    cell = DraggableLabel(cell_text, self, cell_list=self.usrDefined_tab_cells[slide_idx])
                    ...
                    self.usrDefined_tab_cells[slide_idx].append(cell)
                ...
                while cell_idx < grid_size:
                    ...
                    cell = DraggableLabel("Empty", self, cell_list=self.usrDefined_tab_cells[slide_idx])
                    cell.setAlignment(Qt.AlignCenter)
                    cell.setMinimumSize(100, 60)
                    cell.setStyleSheet(
            ...
            self.usrDefined_page_tabs.setVisible(len(slide_axis_values) > 0)
    ...
    def _update_usrDefined_preview_with_tabs(self):
        """Update preview with tabs based on row/column values and axis dropdowns (when DRC plot is not checked)"""
        if not hasattr(self, 'usrDefined_row_inputs') or not hasattr(self, 'usrDefined_column_input'):
            return
        ...
        column_text = self.usrDefined_column_input.text().strip()
        column_values = [v.strip() for v in column_text.split(',') if v.strip()] if column_text else []
        
        row_values_list = []
        for i in range(num_rows):
            if i in self.usrDefined_row_inputs:
                row_text = self.usrDefined_row_inputs[i].text().strip()
    ...
    def _update_usrDefined_axis_dropdowns(self):
        """Update axis dropdowns with available variables from filename template (for DRC plot mode)"""
        if not hasattr(self, 'usrDefined_filename_template_input'):
            return
        ...
        if hasattr(self, 'usrDefined_slide_axis_combo'):
            current_slide = self.usrDefined_slide_axis_combo.currentText()
            self.usrDefined_slide_axis_combo.clear()
            self.usrDefined_slide_axis_combo.addItems([""] + filename_variables)
            if current_slide in filename_variables:
                self.usrDefined_slide_axis_combo.setCurrentText(current_slide)
        
        if hasattr(self, 'usrDefined_grid_axis_combo'):
            current_grid = self.usrDefined_grid_axis_combo.currentText()
            self.usrDefined_grid_axis_combo.clear()
            self.usrDefined_grid_axis_combo.addItems([""] + filename_variables)
            if current_grid in filename_variables:
                self.usrDefined_grid_axis_combo.setCurrentText(current_grid)
    
    def _update_usrDefined_slide_axis_dropdown_normal(self):
        ...
        if hasattr(self, 'usrDefined_slide_axis_combo_normal'):
            current_slide = self.usrDefined_slide_axis_combo_normal.currentText()
            self.usrDefined_slide_axis_combo_normal.clear()
            self.usrDefined_slide_axis_combo_normal.addItem("")  # None option
            self.usrDefined_slide_axis_combo_normal.addItems(filename_variables)
            if current_slide in filename_variables:
                self.usrDefined_slide_axis_combo_normal.setCurrentText(current_slide)
    
    def on_usrDefined_slide_axis_normal_changed(self, text):
    ...
    def show_settings(self):
        ...
        if dialog.exec_():
            ...
            # Note: update_usrDefined_template_info() is not called here to preserve
            ...
            # textChanged signals connected to update_usrDefined_template_info()
    ...
    def generate_slides(self):
        ...
        try:
            ...
            if mode == "cpv":
                ...
                file_format = self.usrDefined_file_format_combo.currentText()
            ...
            # Get selected energy points from preview grid (DRC) or checkboxes (CPV/User-defined)
            if mode == "drc":
            ...
            if mode == "cpv":
                ...
                from Config.Rules.usrDefined_template_engine import create_template_parser
                path_template = self.usrDefined_path_template if hasattr(self, 'usrDefined_path_template') else ""
                filename_template = self.usrDefined_filename_template if hasattr(self, 'usrDefined_filename_template') else ""
                ...
                selected_objects = []
                if has_object:
                    if hasattr(self, 'usrDefined_variable_inputs') and 'object' in self.usrDefined_variable_inputs:
                        object_input = self.usrDefined_variable_inputs['object']
                ...
                # This will be handled in keynoteCtrl_usrDefined.py
                
                # Get selected steps from Variables section (only if step is in template)
                selected_steps = []
                if has_step:
                    if hasattr(self, 'usrDefined_variable_inputs') and 'step' in self.usrDefined_variable_inputs:
                        step_input = self.usrDefined_variable_inputs['step']
                ...
                usrDefined_path_template = self.usrDefined_path_template_input.text().strip()
                usrDefined_filename_template = self.usrDefined_filename_template_input.text().strip()
                usrDefined_variables = {}
                
                # Use default templates if empty
                default_path_template = "{sample}/{channel}/{object}/{step}"
                default_filename_template = "h_{object}_{kinematic}_{step}.{format}"
                
                if not usrDefined_path_template:
                    usrDefined_path_template = default_path_template
                if not usrDefined_filename_template:
                    usrDefined_filename_template = default_filename_template
                
                # Get variable values from UI
                for var_name, var_input in self.usrDefined_variable_inputs.items():
                    # Get from input field (all variables are now in Variables section)
                    var_text = var_input.text().strip()
                    if var_text:
                        # Parse comma-separated values
                        usrDefined_variables[var_name] = [v.strip() for v in var_text.split(',') if v.strip()]
                
                # Get row and column values from preview inputs
                # Get column values (single input)
                column_text = self.usrDefined_column_input.text().strip()
                usrDefined_column_values = [v.strip() for v in column_text.split(',') if v.strip()] if column_text else []
                
                # Get row values from each row input field
                usrDefined_row_values = []
                if hasattr(self, 'usrDefined_row_inputs'):
                    for i in sorted(self.usrDefined_row_inputs.keys()):
                        row_input = self.usrDefined_row_inputs[i]
                        row_text = row_input.text().strip()
                        row_values = [v.strip() for v in row_text.split(',') if v.strip()] if row_text else []
                        usrDefined_row_values.append(row_values)
            
            # Create and start thread
            if mode == "usrDefined":
                # Get scan mode checkbox state
                usrDefined_scan_mode = False
                if hasattr(self, 'usrDefined_scan_mode_check'):
                    usrDefined_scan_mode = self.usrDefined_scan_mode_check.isChecked()
                
                # Get DRC plot mode information
                usrDefined_drc_plot = False
                usrDefined_slide_axis = None
                usrDefined_grid_axis = None
                usrDefined_arrangement = None
                usrDefined_confirmed_scan_results = None
                usrDefined_preview_cell_order = None  # Store preview cell order for each slide
                
                if hasattr(self, 'usrDefined_drc_plot_check'):
                    usrDefined_drc_plot = self.usrDefined_drc_plot_check.isChecked()
                    if usrDefined_drc_plot:
                        if hasattr(self, 'usrDefined_slide_axis_combo'):
                            usrDefined_slide_axis = self.usrDefined_slide_axis_combo.currentText()
                        if hasattr(self, 'usrDefined_grid_axis_combo'):
                            usrDefined_grid_axis = self.usrDefined_grid_axis_combo.currentText()
                        if hasattr(self, 'usrDefined_arrangement_combo'):
                            usrDefined_arrangement = self.usrDefined_arrangement_combo.currentText()
                        if hasattr(self, 'usrDefined_confirmed_scan_results'):
                            usrDefined_confirmed_scan_results = self.usrDefined_confirmed_scan_results
                        
                        # Read preview cell order from tabs for DRC plot mode (preserves drag-and-drop order)
                        if (hasattr(self, 'usrDefined_tab_grids') and 
                            hasattr(self, 'usrDefined_tab_cells') and 
                            hasattr(self, 'usrDefined_page_tabs') and
                            usrDefined_grid_axis and
                            usrDefined_arrangement):
                            usrDefined_preview_cell_order = {}
                            try:
                                num_rows, num_cols = map(int, usrDefined_arrangement.split('*'))
                            except:
                                num_rows, num_cols = 2, 3
                            
                            for slide_idx, tab_grid_layout in enumerate(self.usrDefined_tab_grids):
                                if tab_grid_layout:
                                    ...
                                    for row in range(num_rows):
                                        for col in range(num_cols):
                                            ...
                                            if item and item.widget():
                                                ...
                                                grid_value = None
                                                if cell_text and cell_text != "Empty":
                                                    if f"{usrDefined_grid_axis}=" in cell_text:
                                                        # Format: "grid_axis=value" or "grid_axis=value\n..."
                                                        value_part = cell_text.split(f"{usrDefined_grid_axis}=")[1]
                                    ...
                                    usrDefined_preview_cell_order[slide_idx] = cell_positions
                    else:
                        # Normal mode (DRC plot OFF): Get slide axis from normal dropdown
                        if hasattr(self, 'usrDefined_slide_axis_combo_normal'):
                            slide_axis_text = self.usrDefined_slide_axis_combo_normal.currentText()
                            if slide_axis_text:  # Not empty
                                usrDefined_slide_axis = slide_axis_text
                ...
                self.generator_thread = GeneratorThread(
                    ...
                    self.user_settings, usrDefined_path_template, usrDefined_filename_template, usrDefined_variables,
                    usrDefined_row_values, usrDefined_column_values, usrDefined_scan_mode
                )
                
                # Store DRC plot mode information in thread instance
                self.generator_thread.usrDefined_drc_plot = usrDefined_drc_plot
                self.generator_thread.usrDefined_slide_axis = usrDefined_slide_axis
                self.generator_thread.usrDefined_grid_axis = usrDefined_grid_axis
                self.generator_thread.usrDefined_arrangement = usrDefined_arrangement
                self.generator_thread.usrDefined_confirmed_scan_results = usrDefined_confirmed_scan_results
                self.generator_thread.usrDefined_preview_cell_order = usrDefined_preview_cell_order
            else:
                self.generator_thread = GeneratorThread(
                    mode, systematic, input_dir, output_file,
                    selected_objects, selected_steps, observable_text, jobversion,
    ...
    def _toggle_all_usrDefined_objects(self, checked):
    ...
    def _toggle_all_usrDefined_steps(self, checked):
    ...
    def _update_scan_button_visibility(self):
        ...
        if hasattr(self, 'scan_button'):
            if self.mode == "usrDefined" and hasattr(self, 'usrDefined_scan_mode_check'):
                # Show SCAN button only if scan mode is enabled
                self.scan_button.setVisible(self.usrDefined_scan_mode_check.isChecked())
            else:
                self.scan_button.setVisible(False)
    ...
    def _update_path_template_input_state(self):
        ...
        if hasattr(self, 'usrDefined_path_template_input') and hasattr(self, 'usrDefined_scan_mode_check'):
            # Disable path template input when scan mode is enabled
            scan_mode_enabled = self.usrDefined_scan_mode_check.isChecked()
            self.usrDefined_path_template_input.setEnabled(not scan_mode_enabled)
            
            # Update tooltip to explain why it's disabled
            if scan_mode_enabled:
                self.usrDefined_path_template_input.setToolTip(
        ...
        if hasattr(self, 'usrDefined_scan_mode_check') and hasattr(self, 'usrDefined_path_variables'):
            scan_mode_enabled = self.usrDefined_scan_mode_check.isChecked()
            path_variables = getattr(self, 'usrDefined_path_variables', [])
            
            if hasattr(self, 'usrDefined_variable_inputs'):
                for var_name in path_variables:
                    if var_name in self.usrDefined_variable_inputs:
                        var_input = self.usrDefined_variable_inputs[var_name]
    ...
    def scan_files(self):
        ...
        try:
            ...
            # Perform scan
            from Config.Rules.rules_usrDefined import find_usrDefined_files_scan
            ...
            def on_correct_clicked():
                ...
                self.usrDefined_confirmed_scan_results = self.usrDefined_scan_results
