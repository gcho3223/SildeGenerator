#!/usr/bin/env python3
"""
Loop-defined Mode UI components for Keynote Slide Generator
Loop-defined template-based plot insertion
"""
...
def create_loopDefined_options_ui(parent):
    """
    Create Loop-defined mode options UI
    Returns: (loopDefined_options_group, file_format_combo, path_template_input, filename_template_input)
    """
    loopDefined_options_group = QGroupBox("Loop-defined Options")
    loopDefined_options_layout = QVBoxLayout()
    ...
    loopDefined_options_layout.addLayout(format_layout)
    ...
    loopDefined_options_layout.addLayout(path_template_layout)
    ...
    loopDefined_options_layout.addLayout(filename_template_layout)
    
    loopDefined_options_group.setLayout(loopDefined_options_layout)
    
    return loopDefined_options_group, file_format_combo, path_template_input, filename_template_input, scan_mode_check


def create_loopDefined_objects_ui(parent):
    """
    Create Loop-defined mode object input UI
    Users can directly input object names in text fields
    Returns: (loopDefined_objects_widget, object_inputs)
    """
    loopDefined_objects_widget = QWidget()
    loopDefined_objects_layout = QVBoxLayout()
    loopDefined_objects_layout.setContentsMargins(0, 0, 0, 0)
    
    # Object input field (no label or instructions)
    object_input = QLineEdit()
    object_input.setPlaceholderText("e.g., Lep1,Lep2,Jet1")
    loopDefined_objects_layout.addWidget(object_input)
    
    # Store as dict for consistency with other modes
    object_inputs = {"object": object_input}
    
    loopDefined_objects_widget.setLayout(loopDefined_objects_layout)
    
    return loopDefined_objects_widget, object_inputs


def create_loopDefined_variables_ui(parent):
    """
    Create Loop-defined mode variable values input UI
    Users input values for template variables defined in Settings
    Returns: (loopDefined_variables_widget, variable_inputs)
    """
    loopDefined_variables_widget = QWidget()
    loopDefined_variables_layout = QVBoxLayout()
    loopDefined_variables_layout.setContentsMargins(0, 0, 0, 0)
    
    # Instructions
    info_label = QLabel(
        "Enter values for template variables defined in Settings → Loop-defined tab.\n"
    ...
    loopDefined_variables_layout.addWidget(info_label)
    ...
    loopDefined_variables_layout.addLayout(form_layout)
    loopDefined_variables_layout.addStretch()
    
    loopDefined_variables_widget.setLayout(loopDefined_variables_layout)
    
    return loopDefined_variables_widget, variable_inputs


def create_loopDefined_preview_ui(parent):
    """
    Create Loop-defined mode preview UI with Arrangement selection
...
def update_loopDefined_row_inputs(parent, arrangement):
    """Update row input fields based on arrangement selection"""
    if not hasattr(parent, 'loopDefined_row_inputs_layout'):
        return
    ...
    while parent.loopDefined_row_inputs_layout.count():
        item = parent.loopDefined_row_inputs_layout.takeAt(0)
        if item.layout():
            ...
            while layout.count():
                ...
                if sub_item.widget():
                    ...
                    if widget != parent.loopDefined_column_input:
                        widget.deleteLater()
        elif item.widget() and item.widget() != parent.loopDefined_column_input:
            item.widget().deleteLater()
    
    # Clear row inputs dictionary
    if hasattr(parent, 'loopDefined_row_inputs'):
        parent.loopDefined_row_inputs.clear()
    else:
        parent.loopDefined_row_inputs = {}
    ...
    for i in range(rows):
        ...
        if i == 0:
            ...
            if hasattr(parent, 'loopDefined_column_input'):
                row_layout.addWidget(parent.loopDefined_column_input)
        
        row_layout.addStretch()
        parent.loopDefined_row_inputs_layout.addLayout(row_layout)
        parent.loopDefined_row_inputs[i] = row_input
        # Connect to update preview
        row_input.textChanged.connect(parent.update_loopDefined_preview)
    
    # Add Slide axis dropdown below column (after last row)
    if not hasattr(parent, 'loopDefined_slide_axis_combo_normal') or parent.loopDefined_slide_axis_combo_normal is None:
        ...
        parent.loopDefined_row_inputs_layout.addLayout(slide_axis_layout)
        parent.loopDefined_slide_axis_combo_normal = slide_axis_combo_normal
        # Connect to update preview and axis dropdowns
        slide_axis_combo_normal.currentTextChanged.connect(parent.on_loopDefined_slide_axis_normal_changed)
        # Update dropdown items when template changes
        if hasattr(parent, 'loopDefined_filename_template_input'):
            parent.loopDefined_filename_template_input.textChanged.connect(
                lambda: parent._update_loopDefined_slide_axis_dropdown_normal()
            )
    else:
        # Slide axis combo already exists, just update it
        parent.loopDefined_slide_axis_combo_normal.setParent(None)
        slide_axis_layout = QHBoxLayout()
        slide_axis_label = QLabel("Slide axis:")
        slide_axis_layout.addWidget(slide_axis_label)
        slide_axis_layout.addWidget(parent.loopDefined_slide_axis_combo_normal)
        slide_axis_layout.addStretch()
        parent.loopDefined_row_inputs_layout.addLayout(slide_axis_layout)
    
    # Update dropdown items
    if hasattr(parent, '_update_loopDefined_slide_axis_dropdown_normal'):
        parent._update_loopDefined_slide_axis_dropdown_normal()


def update_loopDefined_preview(parent):
    """Update the preview layout in Loop-defined mode based on row and column inputs"""
    # Check if DRC plot is checked (if checked, don't use normal preview)
    drc_plot_checked = False
    if hasattr(parent, 'loopDefined_drc_plot_check'):
        drc_plot_checked = parent.loopDefined_drc_plot_check.isChecked()
    
    # If DRC plot is checked and has scan results, use DRC preview instead
    if drc_plot_checked:
        if hasattr(parent, 'loopDefined_confirmed_scan_results') and parent.loopDefined_confirmed_scan_results:
            # Use DRC preview - don't show normal preview
            if hasattr(parent, '_update_loopDefined_drc_preview'):
                parent._update_loopDefined_drc_preview()
            # Hide main grid
            if hasattr(parent, 'loopDefined_preview_group'):
                preview_layout = parent.loopDefined_preview_group.layout()
                if preview_layout:
                    for i in range(preview_layout.count()):
                        item = preview_layout.itemAt(i)
                        if item and item.widget():
                            widget = item.widget()
                            if hasattr(widget, 'layout') and widget.layout() == parent.loopDefined_preview_grid_layout:
    ...
    if hasattr(parent, 'loopDefined_page_tabs'):
        parent.loopDefined_page_tabs.setVisible(False)
    
    # Use normal preview with row/column inputs
    if not hasattr(parent, 'loopDefined_row_inputs') or not hasattr(parent, 'loopDefined_column_input'):
        return
    
    if not hasattr(parent, 'loopDefined_preview_grid_layout'):
        return
    
    # Get arrangement
    if hasattr(parent, 'loopDefined_arrangement_combo'):
        arrangement = parent.loopDefined_arrangement_combo.currentText()
        try:
    ...
    # Get column values
    column_text = parent.loopDefined_column_input.text().strip()
    column_values = [v.strip() for v in column_text.split(',') if v.strip()] if column_text else []
    
    # Get row values from each row input
    row_values_list = []
    for i in range(num_rows):
        if i in parent.loopDefined_row_inputs:
            row_text = parent.loopDefined_row_inputs[i].text().strip()
    ...
    if hasattr(parent, 'loopDefined_preview_cells'):
        for cell in parent.loopDefined_preview_cells:
            if cell:
                cell.deleteLater()
        parent.loopDefined_preview_cells.clear()
    else:
        parent.loopDefined_preview_cells = []
    
    # Clear grid layout
    while parent.loopDefined_preview_grid_layout.count():
        item = parent.loopDefined_preview_grid_layout.takeAt(0)
        if item.widget():
            item.widget().deleteLater()
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            cell = DraggableLabel(cell_text, parent, cell_list=parent.loopDefined_preview_cells)
            ...
            parent.loopDefined_preview_grid_layout.addWidget(cell, row, col)
            parent.loopDefined_preview_cells.append(cell)
...
# Loop-defined Steps UI functions removed
# Steps are now handled directly in Variables section
# No separate Steps UI needed


def create_loopDefined_energy_checkboxes(parent, energy_values):
    """
    Create energy checkboxes for DRC-style preview in Loop-defined mode
    Similar to create_energy_checkboxes in gui_drc.py
    """
    if not hasattr(parent, 'loopDefined_energy_checkboxes_layout'):
        return
    
    # Clear existing checkboxes
    while parent.loopDefined_energy_checkboxes_layout.count():
        child = parent.loopDefined_energy_checkboxes_layout.takeAt(0)
        if child.widget():
    ...
    if not hasattr(parent, 'loopDefined_energy_checks'):
        parent.loopDefined_energy_checks = {}
    else:
        parent.loopDefined_energy_checks.clear()
    ...
    for row in energy_values:
        ...
        for energy in row:
            ...
            parent.loopDefined_energy_checks[str(energy)] = check
            # Connect to update preview
            check.stateChanged.connect(lambda state, p=parent: update_loopDefined_drc_preview(p))
            row_layout.addWidget(check)
        row_layout.addStretch()
        parent.loopDefined_energy_checkboxes_layout.addLayout(row_layout)


def update_loopDefined_drc_preview(parent):
    """Update the DRC-style energy preview layout in Loop-defined mode"""
    if not hasattr(parent, 'loopDefined_energy_checks'):
        return
    ...
    selected_energies = [energy for energy, check in parent.loopDefined_energy_checks.items() if check.isChecked()]
...
def run_loopDefined_generation(thread_instance):
    """
    Run Loop-defined mode slide generation
    ...
    try:
        ...
        from Config.KeynoteCtrl.keynoteCtrl_loopDefined import insert_pdfs_into_slide_loopDefined
        from Config.Rules.loopDefined_template_engine import create_template_parser, validate_template
        from Config.Rules.rules_loopDefined import find_loopDefined_files, find_loopDefined_files_scan
        
        # Check scan mode
        scan_mode = getattr(thread_instance, 'loopDefined_scan_mode', False)
        ...
        path_template = thread_instance.loopDefined_path_template
        filename_template = thread_instance.loopDefined_filename_template
        variable_combinations = thread_instance.loopDefined_variables
        
        if not filename_template:
            return False, "Filename template not configured. Please set filename template in Settings → Loop-defined tab."
        ...
        if not scan_mode and not path_template:
            return False, "Path template not configured. Please set path template in Settings → Loop-defined tab."
        ...
        if scan_mode:
            ...
            # Store scan results for use in insert_pdfs_into_slide_loopDefined
        ...
        drc_plot = getattr(thread_instance, 'loopDefined_drc_plot', False)
        slide_axis = getattr(thread_instance, 'loopDefined_slide_axis', None)
        grid_axis = getattr(thread_instance, 'loopDefined_grid_axis', None)
        arrangement = getattr(thread_instance, 'loopDefined_arrangement', None)
        confirmed_scan_results = getattr(thread_instance, 'loopDefined_confirmed_scan_results', None)
        preview_cell_order = getattr(thread_instance, 'loopDefined_preview_cell_order', None)
        ...
        insert_pdfs_into_slide_loopDefined(
            ...
            row_values=thread_instance.loopDefined_row_values,
            column_values=thread_instance.loopDefined_column_values,
