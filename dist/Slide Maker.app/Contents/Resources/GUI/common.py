    ...
    def dropEvent(self, event):
        ...
        if mime_data.hasText():
            ...
            if source_cell:
                ...
                # Swap file paths (handle cases where only one has file_path)
                self_has_path = hasattr(self, 'file_path')
                source_has_path = hasattr(source_cell, 'file_path')
                
                if self_has_path and source_has_path:
                    # Both have paths - swap them
                    self.file_path, source_cell.file_path = source_cell.file_path, self.file_path
                elif self_has_path and not source_has_path:
                    # Only self has path - move to source
                    source_cell.file_path = self.file_path
                    delattr(self, 'file_path')
                elif not self_has_path and source_has_path:
                    # Only source has path - move to self
                    self.file_path = source_cell.file_path
                    delattr(source_cell, 'file_path')
                
                # Update dragdrop_file_paths dictionary if in dragdrop mode
                if self.parent_widget and hasattr(self.parent_widget, 'dragdrop_file_paths'):
                    # Get cell indices for both cells
                    self_slide_idx = getattr(self, 'slide_idx', None)
                    self_cell_idx = getattr(self, 'cell_index', None)
                    source_slide_idx = getattr(source_cell, 'slide_idx', None)
                    source_cell_idx = getattr(source_cell, 'cell_index', None)
                    
                    if (self_slide_idx is not None and self_cell_idx is not None and 
                        source_slide_idx is not None and source_cell_idx is not None):
                        # Both cells have slide/cell indices - swap their entries in dictionary
                        if self_slide_idx not in self.parent_widget.dragdrop_file_paths:
                            self.parent_widget.dragdrop_file_paths[self_slide_idx] = {}
                        if source_slide_idx not in self.parent_widget.dragdrop_file_paths:
                            self.parent_widget.dragdrop_file_paths[source_slide_idx] = {}
                        
                        # Get current values
                        self_path = self.parent_widget.dragdrop_file_paths[self_slide_idx].get(self_cell_idx)
                        source_path = self.parent_widget.dragdrop_file_paths[source_slide_idx].get(source_cell_idx)
                        
                        # Swap values
                        if source_path is not None:
                            self.parent_widget.dragdrop_file_paths[self_slide_idx][self_cell_idx] = source_path
                        elif self_cell_idx in self.parent_widget.dragdrop_file_paths[self_slide_idx]:
                            del self.parent_widget.dragdrop_file_paths[self_slide_idx][self_cell_idx]
                        
                        if self_path is not None:
                            self.parent_widget.dragdrop_file_paths[source_slide_idx][source_cell_idx] = self_path
                        elif source_cell_idx in self.parent_widget.dragdrop_file_paths[source_slide_idx]:
                            del self.parent_widget.dragdrop_file_paths[source_slide_idx][source_cell_idx]
